module Test::SurveysHelper
  ...
  def new_survey
    new_test_survey_path
  end

  def edit_survey(resource)
    edit_test_survey_path(resource)
  end

  def survey_scope(resource)
    if action_name =~ /new|create/
      test_surveys_path(resource)
    elsif action_name =~ /edit|update/
      test_survey_path(resource)
    end
  end

  def new_attempt
    new_test_attempt_path
  end

  def attempt_scope(resource)
    if action_name =~ /new|create/
     test_attempts_path(resource)
    elsif action_name =~ /edit|update/
      test_attempt_path(resource)
    end
  end
